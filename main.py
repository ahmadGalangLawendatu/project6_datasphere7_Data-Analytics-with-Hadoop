...
if __name__ == '__main__':
    ...
    if 'user.name' in conf_hadoop:
        original_username = conf_hadoop['user.name']
        conf_hadoop['user.name'] = conf_hadoop['user.name'].replace(' ', '_')
        print(f'[DEBUG] Original user.name: {original_username}, Modified user.name: {conf_hadoop["user.name"]}')
    ...
    try:
        ...
        with client.write('/digitalskola/project/dim_orders_galang.csv', encoding='utf-8') as writer:
